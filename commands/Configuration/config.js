module.exports = {
	...
	execute: async (bot, msg, args)=> {
		...
		var bchan = conf.banlog_channel ? await msg.guild.channels.fetch(conf.banlog_channel) : undefined;
		var rrole = conf.reprole ?  await msg.guild.roles.fetch(conf.reprole) : undefined;
		var dchan = conf.delist_channel ? await msg.guild.channels.fetch(conf.delist_channel) : undefined;
		var echan = conf.edit_channel ? await msg.guild.channels.fetch(conf.edit_channel) : undefined;

		return {embed: {
			title: "Server Config",
			fields: [
				{name: "Ban Log Channel", value: bchan ? bchan.toString() : "(not set)"},
				{name: "Ban Message", value: conf.ban_message || "(not set)"},
				{name: "Delist/Denial Log Channel", value: dchan ? dchan.toString() : "(not set)"},
				{name: "Representative Role", value: rrole ? rrole.toString() : "(not set)"},
				{name: "Edit Requests Channel", value: echan ? echan.toString() : "(not set)"}
			]
		}};
	},
	alias: ['conf','cfg'],
...
module.exports.subcommands.banlog = {
	...
	execute: async (bot, msg, args)=> {
		...
		var arg = args[0].replace(/[<#>]/g,"").toLowerCase();
		var chan = msg.guild.channels.cache.find(ch => [ch.id, ch.name].includes(arg));
		if(!chan || chan.type != 'text') return "Channel not found";
